...
const tenantConfigModel = require('./tenantConfig.model');
...
const list = async (moduleName, logger) => {
  ...
  try {
    const tenants = await tenantConfigModel.list();
    if (tenants) {
...
const findById = async (moduleName, id, logger) => {
  ...
  try {
    const tempTenants = await tenantConfigModel.findById(id, logger);
...
const create = async (moduleName, tenantData, logger) => {
  ...
  try {
    ...
    const tenantResult = await tenantConfigModel.create(newTenantData);
