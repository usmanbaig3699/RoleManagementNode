...
const moduleScheduleName = 'Provider Schedule';
...
const find = async (req, res) => {
  logger.verbose(`Handling ${req.method} ${req.url} Route`);
  try {
    const data = await Service.find(moduleName, req.params.id, logger);
    if (data && !data.hasError) {
      logger.verbose(
        `Handling Completed With Success On ${req.method} ${req.url} Route`
      );
...
const findAppointmentByProviderId = async (req, res) => {
  logger.verbose(`Handling ${req.method} ${req.url} Route`);
  try {
    const data = await Service.findAppointmentByProviderId(moduleName, req.params, req.query, logger);
    if (data && !data.hasError) {
      logger.verbose(
        `Handling Completed With Success On ${req.method} ${req.url} Route`
      );
...
const del = async (req, res) => {
  logger.verbose(`Handling ${req.method} ${req.url} Route`);
  try {
    const data = await Service.del(moduleName, req.params.id, req.body, logger);
    if (data && !data.hasError) {
      logger.verbose(
        `Handling Completed With Success On ${req.method} ${req.url} Route`
      );
...
const scheduleList = async (req, res) => {
  logger.verbose(`Handling ${req.method} ${req.url} Route`);
  try {
    const data = await Service.scheduleList(
      moduleScheduleName,
      req.params,
      logger
    );
    ...
    logger.error(
      `Error in calling ${moduleScheduleName} list service.
      Error:: ${error}
      Trace:: ${error.stack}`
    );
    return res.status(HTTP_STATUS.OK).send(
...
const scheduleCreate = async (req, res) => {
  logger.verbose(`Handling ${req.method} ${req.url} Route`);
  try {
    const data = await Service.scheduleCreate(
      moduleScheduleName,
      req.body,
      logger
    );
    ...
    logger.error(
      `Error in calling ${moduleScheduleName} create service.
      Error:: ${error}
      Trace:: ${error.stack}`
    );
    return res.status(HTTP_STATUS.OK).send(
...
const scheduleFind = async (req, res) => {
  logger.verbose(`Handling ${req.method} ${req.url} Route`);
  try {
    const data = await Service.scheduleFind(
      moduleScheduleName,
      req.params,
      logger
    );
    ...
    logger.error(
      `Error in calling ${moduleScheduleName} find service.
      Error:: ${error}
      Trace:: ${error.stack}`
    );
    return res.status(HTTP_STATUS.OK).send(
...
const scheduleUpdate = async (req, res) => {
  logger.verbose(`Handling ${req.method} ${req.url} Route`);
  try {
    const data = await Service.scheduleUpdate(
      moduleScheduleName,
      req.body,
      logger
    );
    ...
    logger.error(
      `Error in calling ${moduleScheduleName} update service.
      Error:: ${error}
      Trace:: ${error.stack}`
    );
    return res.status(HTTP_STATUS.OK).send(
...
const scheduleUpdateStatus = async (req, res) => {
  ...
  try {
    ...
    logger.error(
      `Error in calling ${moduleScheduleName} update status service.
      Error:: ${error}
      Trace:: ${error.stack}`
    );
    return res.status(HTTP_STATUS.OK).send(
